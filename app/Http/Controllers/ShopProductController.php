...
class ShopProductController extends RootFrontController
{
    ...
    private function _productDetail($alias)
    {
     ...
     /**
     * Get product detail - quick view
     *
     * @param   [string]  $alias      [$alias description]
     *
     * @return  [mix]
     */
    private function _productDetailQuick($alias)
    {
        $storeId = config('app.storeId');
        $product = (new ShopProduct)->getDetail($alias, $type = 'alias', $storeId);
        if ($product && $product->status && (!sc_config('product_stock', $storeId) || sc_config('product_display_out_of_stock', $storeId) || $product->stock > 0)) {
            ...
            sc_check_view($this->templatePath . '.common.product_detail_modal');
            return view(
                $this->templatePath . '.common.product_detail_modal',
                array(
                    'product'         => $product,
                    // 'layout_page'     => 'shop_product_detail',
                )
      ...
      /**
     * Process front product detail - quick view (modal)
     *
     * @param [type] ...$params
     * @return void
     */
    public function productDetailProcessModal(...$params)
    {
        if (config('app.seoLang')) {
            $lang = $params[0] ?? '';
            $alias = $params[1] ?? '';
        ...
        return $this->_productDetailQuick($alias);
    }
