     ...
     <div class="container">
         <div class="row row-30">
             <div class="col-lg-6">
                 @if (!$modal)
                     <div class="slick-vertical slick-product">
                         ...
                         <div class="slick-slider carousel-parent" id="carousel-parent" data-items="1" data-swipe="true"
                             ...
                             <div class="item">
                                 <div class="slick-product-figure"><img src="{{ sc_file($product->getImage()) }}"
                             ...
                             @if ($product->images->count())
                                 ...
                                 @foreach ($product->images as $key => $image)
                                     <div class="item">
                                         <div class="slick-product-figure"><img src="{{ sc_file($image->getImage()) }}"
                                                 alt="" width="530" height="480" />
                                         </div>
                                     </div>
                                 @endforeach
                         ...
                         @if ($countItem > 1)
                             <div class="slick-slider child-carousel slick-nav-1" id="child-carousel" data-arrows="true"
                                 data-items="{{ $countItem }}" data-sm-items="{{ $countItem }}"
                                 data-md-items="{{ $countItem }}" data-lg-items="{{ $countItem }}"
                                 data-xl-items="{{ $countItem }}" data-xxl-items="{{ $countItem }}"
                                 ...
                                 @foreach ($product->images as $key => $image)
                                     <div class="item">
                                         <div class="slick-product-figure"><img src="{{ sc_file($image->getThumb()) }}"
                                                 alt="" width="530" height="480" />
                                         </div>
                                     </div>
                                 @endforeach
                 ...
                 @else
                     <div id="{{ $product->images->count() > 1 ? 'productImageSlider' : '' }}" style="height:100%">
                         <div><img src="{{ sc_file($product->getImage()) }}" alt="product-image" width="530"
                                 height="480" />
                         </div>
                         @if ($product->images->count() > 0)
                             @foreach ($product->images as $key => $image)
                                 <div><img src="{{ sc_file($image->getImage()) }}" alt="" width="530"
                                         height="480" />
                                 </div>
                             @endforeach
                         @endif
                             ...
                             <div class="group-xs group-middle">
                                 ...
                                 <div>
                                     <button class="button button-lg button-secondary button-zakaria"
                                         type="submit">{{ sc_language_render('action.add_to_cart') }}</button>
                                 </div>
                             </div>
                         @endif
                         {{-- // Button add to cart --}}
                         ...
                         @if (sc_config('product_brand') && !empty($product->brand->name))
                             <div>
                                 ...
                                 <span id="product-detail-brand">
                                     {!! empty($product->brand->name)
                                         ? 'None'
                                         : '<a href="' . $product->brand->getUrl() . '">' . $product->brand->name . '</a>' !!}
                                 </span>
                             </div>
                         @endif
                         {{-- // Brand info --}}
                             ...
                             <div class="products-group">
                                 ...
                                 @foreach ($builds as $k => $build)
                                     ...
                                     <span class="sc-product-build">{{ $build->quantity }} x
                                         <a target="_new"
                                             href="{{ $build->product->getUrl() }}">{!! sc_image_render($build->product->image) !!}</a>
                                     </span>
                                 @endforeach
                             </div>
                         @endif
                 ...
                 <ul class="nav nav-tabs nav-tabs-1">
                     <li class="nav-item" role="presentation">
                         <a class="nav-link active" href="#tabs-1-1"
                             data-toggle="tab">{{ sc_language_render('product.description') }}</a>
                     </li>
                 </ul>
             </div>
 ...
 <script>
     $(document).ready(function() {
         $('#productImageSlider').bbslider({
             auto: true,
             timer: 3000,
             loop: true,
             autoHeight: false,
             controls: true,
             maskImage: '{{ asset('/images/mask.png') }}'
         });

         $('input[name=qty]').on('change', function(change) {
             console.log(change);
         })
     });
 </script>
